...
`demoproject_devops_project1` is a complete teaching repository that provisions, configures, builds, publishes, deploys, and verifies an NGINX web application. Terraform creates two Ubuntu EC2 instances and a private ECR repository; Ansible configures Jenkins and its build toolchain; Jenkins builds the Docker image and publishes it to ECR using an EC2 IAM role; the resulting container serves **Welcome to DEMO Project** on port 80.
